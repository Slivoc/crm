from flask import Blueprint, render_template, request, jsonify, url_for, redirect, session, flash, abort, Response, g, current_app, send_file
...
def _supplier_quote_source_dir():
    folder = os.path.join(current_app.config.get('UPLOAD_FOLDER', 'uploads'), 'supplier_quote_sources')
    os.makedirs(folder, exist_ok=True)
    return folder


def _store_supplier_quote_source(content, original_filename, content_type, source_kind):
    """Store an immutable extraction source and return portable database metadata."""
    if not content:
        return None
    if len(content) > SUPPLIER_QUOTE_SOURCE_MAX_BYTES:
        raise ValueError("Supplier quote source exceeds the 20 MB limit")

    safe_original = secure_filename(original_filename or 'supplier-quote-source') or 'supplier-quote-source'
    extension = os.path.splitext(safe_original)[1].lower()
    storage_key = f"{uuid.uuid4().hex}{extension}"
    full_path = os.path.join(_supplier_quote_source_dir(), storage_key)
    with open(full_path, 'xb') as source_file:
...
def _validated_supplier_quote_source(source):
    """Accept only source metadata generated by this server and still present on disk."""
    if not isinstance(source, dict):
        return None
    storage_key = os.path.basename(str(source.get('path') or ''))
    if not storage_key or storage_key != str(source.get('path') or ''):
        return None
    full_path = os.path.join(_supplier_quote_source_dir(), storage_key)
    ...
    return {
        'path': storage_key,
        'filename': secure_filename(source.get('filename') or storage_key) or storage_key,
        'content_type': str(source.get('content_type') or 'application/octet-stream')[:255],
        'kind': str(source.get('kind') or 'unknown')[:50],
...
def _get_customer_quote_line_metadata(cur, parts_list_line_id, supplier_id=None, source_type=None, source_reference=None):
    ...
    if source_type_value == 'quote' and source_reference is not None:
        ...
        if explicit_quote:
            ...
            cage_code = (explicit_quote['cage_code'] or '').strip()
            test_certs = (explicit_quote['test_certs'] or '').strip()
...
def _sync_customer_quote_line_metadata(cur, parts_list_line_id, supplier_id=None, source_type=None, source_reference=None, clear=False):
    ...
    if clear:
        metadata = {'condition': '', 'certs': '', 'manufacturer': '', 'cage_code': '', 'test_certs': ''}
    else:
...
def create_supplier_quote(list_id):
    ...
    try:
        ...
        source_artifact = _validated_supplier_quote_source(data.get('source_artifact'))
        ...
        row = db_execute(
            ...
            (parts_list_id, supplier_id, quote_reference, quote_date,
             ...
             source_artifact_path, source_artifact_filename, source_artifact_content_type,
             source_artifact_kind, source_artifact_size, source_artifact_sha256)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            RETURNING id
            """,
            (
                ...
                source_artifact.get('path') if source_artifact else None,
                source_artifact.get('filename') if source_artifact else None,
                source_artifact.get('content_type') if source_artifact else None,
                source_artifact.get('kind') if source_artifact else None,
                source_artifact.get('size') if source_artifact else None,
                source_artifact.get('sha256') if source_artifact else None,
            ),
            fetch='one',
            commit=True,
        )
...
@parts_list_bp.route('/parts-lists/<int:list_id>/supplier-quotes/<int:quote_id>/source', methods=['GET'])
def download_supplier_quote_source(list_id, quote_id):
    source = db_execute(
        """
        SELECT source_artifact_path, source_artifact_filename, source_artifact_content_type
    ...
    if not source or not source['source_artifact_path']:
        abort(404)

    storage_key = os.path.basename(source['source_artifact_path'])
    if storage_key != source['source_artifact_path']:
        abort(404)
    full_path = os.path.join(_supplier_quote_source_dir(), storage_key)
    if not os.path.isfile(full_path):
        abort(404)
    ...
    return send_file(
        full_path,
        mimetype=source['source_artifact_content_type'] or 'application/octet-stream',
        as_attachment=True,
        download_name=source['source_artifact_filename'] or storage_key,
    )
    ...
    try:
        ...
        if 'source_artifact' in data:
            ...
            if not source_artifact:
                return jsonify(success=False, message="Source artifact is invalid or no longer available"), 400
            for column, key in (
                ('source_artifact_path', 'path'),
                ('source_artifact_filename', 'filename'),
                ('source_artifact_content_type', 'content_type'),
                ('source_artifact_kind', 'kind'),
                ('source_artifact_size', 'size'),
                ('source_artifact_sha256', 'sha256'),
            ):
                fields.append(f"{column} = ?")
                params.append(source_artifact[key])
...
def save_supplier_quote_lines(list_id, quote_id):
    ...
    try:
        ...
        for idx, line in enumerate(lines):
            ...
            cage_code_raw = line.get('cage_code')
            test_certs_raw = line.get('test_certs')
            ...
            cage_code = _normalize_optional_text(cage_code_raw)
            test_certs = _normalize_optional_text(test_certs_raw)
            ...
            has_quote_data = (
                    ...
                    (cage_code and cage_code.strip()) or
                    (test_certs and test_certs.strip()) or
            ...
            if existing:
                ...
                db_execute(
                    ...
                    (
                        ...
                        lead_time_days, condition_code, certifications, cage_code, test_certs,
                ...
                db_execute(
                    ...
                    (supplier_quote_id, parts_list_line_id, quoted_part_number,
                     ...
                     lead_time_days, condition_code, certifications, cage_code, test_certs, is_no_bid, line_notes)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
    ...
    try:
        ...
        if quote_text.strip():
            source_artifact = _store_supplier_quote_source(
                ...
                f"supplier-email-body-{datetime.now().strftime('%Y%m%d-%H%M%S')}.txt",
...
- is_no_bid: true if supplier declined to quote this part, false otherwise
  ...
  - notes: Any additional relevant notes about this line. Include the originally requested part number here if it differs from what the supplier is quoting. Put DFARS/ITAR compliance and paperwork details other than manufacturer/trace certs or test certs here.
